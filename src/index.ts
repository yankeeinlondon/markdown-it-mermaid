import mdi, { PluginWithOptions } from "markdown-it"
import { uuid } from "native-dash";
import mermaid, { C4Config, Config, FlowChartConfig, GanttConfig,  SequenceConfig } from "mermaid";
...
export interface MermaidOptions {
  /**
   * Provide a mermaid theme or provide two: one for light mode, the latter for dark mode
   */
  theme?: MermaidTheme | [MermaidTheme, MermaidTheme],
  fontFamily?: string,
...
type MermaidConfig = Config & { id?: string }

const MermaidPlugin: PluginWithOptions<MermaidConfig> = (md: MdiWithMermaid, config: MermaidConfig = {}) => {
  ...
  md.renderer.rules.fence = function (tokens, idx, opts, env, self) {
    ...
    if (token.tag === "code" && token.info.startsWith("mermaid")) {
      ...
      try {
        const block = mermaid.render(id, token.content);
